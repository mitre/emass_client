...
class PoamResponseDelete(BaseModel):
    """
    PoamResponseDelete
    """
    meta: Optional[Response200] = None
    data: Optional[conlist(PoamPostPutDel)] = None
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PoamResponseDelete:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> PoamResponseDelete:
        """Create an instance of PoamResponseDelete from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PoamResponseDelete.parse_obj(obj)

        _obj = PoamResponseDelete.parse_obj({
            "meta": Response200.from_dict(obj.get("meta")) if obj.get("meta") is not None else None,
            "data": [PoamPostPutDel.from_dict(_item) for _item in obj.get("data")] if obj.get("data") is not None else None
        })
        return _obj
