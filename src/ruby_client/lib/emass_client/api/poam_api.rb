...
module EmassClient
  class POAMApi
    ...
    # @param system_id [Integer] **System Id**: The unique system record identifier.
    # @param delete_poams_inner [Array<DeletePoamsInner>] Delete the given POA&amp;M Id
    # @param [Hash] opts the optional parameters
    # @return [PoamResponseDelete]
    def delete_poam(system_id, delete_poams_inner, opts = {})
      data, _status_code, _headers = delete_poam_with_http_info(system_id, delete_poams_inner, opts)
      data
    end
    ...
    def delete_poam_with_http_info(system_id, delete_poams_inner, opts = {})
      if @api_client.config.debugging
      ...
      # verify the required parameter 'delete_poams_inner' is set
      if @api_client.config.client_side_validation && delete_poams_inner.nil?
        fail ArgumentError, "Missing the required parameter 'delete_poams_inner' when calling POAMApi.delete_poam"
      end
      # resource path
      local_var_path = '/api/systems/{systemId}/poams'.sub('{' + 'systemId' + '}', CGI.escape(system_id.to_s))
      ...
      # http body (model)
      post_body = opts[:debug_body] || @api_client.object_to_http_body(delete_poams_inner)
