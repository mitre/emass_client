...
module EmassClient
  class MilestonesApi
    ...
    # @param system_id [Integer] **System Id**: The unique system record identifier.
    # @param poam_id [Integer] **POA&amp;M Id**: The unique POA&amp;M record identifier.
    # @param inline_object1 [Array<InlineObject1>] Delete the given Milestone Id
    # @param [Hash] opts the optional parameters
    # @return [MilestonesPutPostDelete]
    def delete_milestone(system_id, poam_id, inline_object1, opts = {})
      data, _status_code, _headers = delete_milestone_with_http_info(system_id, poam_id, inline_object1, opts)
      data
    end
    ...
    def delete_milestone_with_http_info(system_id, poam_id, inline_object1, opts = {})
      if @api_client.config.debugging
      ...
      if @api_client.config.client_side_validation && inline_object1.nil?
        fail ArgumentError, "Missing the required parameter 'inline_object1' when calling MilestonesApi.delete_milestone"
      end
      # resource path
      local_var_path = '/api/systems/{systemId}/poams/{poamId}/milestones'.sub('{' + 'systemId' + '}', CGI.escape(system_id.to_s)).sub('{' + 'poamId' + '}', CGI.escape(poam_id.to_s))
